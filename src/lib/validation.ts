...
function normalizeMessagedAt(value: string): string {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) {
    throw new Error("Enter the exact date and time you messaged us.");
  }
  return date.toISOString();
}

export function validateHistoricalEntryInput(
  input: unknown,
): HistoricalEntryInput {
  if (!input || typeof input !== "object") {
    throw new Error("Invalid submission.");
  }
  ...
  const note = typeof raw.note === "string" ? raw.note.trim() : "";
  const messagedAtRaw =
    typeof raw.messagedAt === "string" ? raw.messagedAt.trim() : "";
  ...
  return {
    redditUsername,
    whatsapp: digits,
    messagedAt: normalizeMessagedAt(messagedAtRaw),
    note,
  };
}

export function validateHistoricalPartialEntryInput(
  input: unknown,
): Partial<HistoricalEntryInput> {
  if (!input || typeof input !== "object") {
    throw new Error("Invalid data.");
  }

  const raw = input as Record<string, unknown>;
  const patch: Partial<HistoricalEntryInput> = {};
  ...
  if (raw.messagedAt !== undefined) {
    const value = typeof raw.messagedAt === "string" ? raw.messagedAt.trim() : "";
    if (!value) {
      throw new Error("Enter the exact date and time you messaged us.");
    }
    patch.messagedAt = normalizeMessagedAt(value);
  }
